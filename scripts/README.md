...
| `make_stub_dataset.py` | 실제 데이터 없이 smoke test용 NPZ를 생성합니다. |
| `comparison_table.py` | S1, S2, S3 평가 결과를 하나의 비교표로 만듭니다. |
...
스크립트는 저장소 루트에서 실행하는 것을 전제로 하며, 기본 입출력 경로는 `data/`와 `results/`입니다.
